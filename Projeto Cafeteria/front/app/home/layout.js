...
// Nested layouts in the Next.js app router should NOT render <html> or <body>.
// Only the root layout (app/layout.js) owns those tags.
export default function RootLayout({ children }) {
  const [nome, setNome] = useState("");

  useEffect(() => {
    try {
      const stored = localStorage.getItem("nome");
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-zinc-100">
      ...
      <header className="w-full bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-6 py-3 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="text-sm text-zinc-600 dark:text-zinc-400">Bem-vindo{nome ? `, ${nome}` : ''}</div>
        </div>
      </header>
